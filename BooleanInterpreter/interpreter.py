"""BooleanInterpreter interprets boolean expressions according
    to a given grammar
...
class Token(object):
    ...
    `Token` is used to represent Non terminals and terminals
    as type-pair objects.

    Attributes:
            :token_type: the token type represents the (non)terminal's type
                        in the grammar rules it belongs to
    ...
    def imply_tail(self):
        ...
        if self.current_token.token_type == IMPLY:
            ...
            if self.or_term():
                ...
                return False
        elif self.current_token.token_type in (EOF, RPAREN):  # selection set
            return True
        else:
